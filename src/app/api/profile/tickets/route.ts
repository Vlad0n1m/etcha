 ...
 * Get all NFT tickets from database for the user (simplified, no signature required)
 */
    ...
    try {
        ...
        if (!walletAddress) {
            return NextResponse.json(
                { success: false, message: 'wallet parameter is required' },
                { status: 400 }
            )
        }
        ...
        const transformedTickets = tickets.map(ticket => {
            ...
            return {
                ...
                price: listing ? listing.price : pricePerTicket,
        ...
        console.error('Error fetching user tickets (GET):', error)
        return NextResponse.json(
            {
                success: false,
