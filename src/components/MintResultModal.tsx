...
export default function MintResultModal({ open, onClose, result, event }: MintResultModalProps) {
    ...
    const totalPaid = result ? result.organizerPayment.amount + result.platformFee.amount : 0
    const network = process.env.NEXT_PUBLIC_SOLANA_NETWORK || "devnet"
    const explorerUrl = result ? `https://explorer.solana.com/tx/${result.transactionSignature}?cluster=${network}` : ""
    ...
    // Only show share prompt if no result but share is open
